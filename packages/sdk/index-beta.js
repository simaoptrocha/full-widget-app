...
function loadCustomWidget() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        return new Promise((resolve, reject) => {
            const scriptSrc = "https://main.d2a9fj2wdht669.amplifyapp.com/index.js";
            ...
            const checkLoaded = (count = 0) => {
                ...
                if (window.SimpleWidgetLoader) {
                    ...
                    // Wrap the original init function to send handshake after the widget is shown.
                    const originalInit = window.SimpleWidgetLoader.init;
                    const wrappedInit = (options) => {
                        const widgetInstance = originalInit(options);
                        // Call show() so the widget (iframe) is rendered
                        // const instance = widgetInstance.show();
                        // After a short delay, send the handshake message from the parent.
                        setTimeout(() => {
                            ...
                            if (instance && instance.targetWindow) {
                                ...
                                }, '*'); // Use specific target origin if known.
                            }
                            else {
                                console.warn("Widget instance or targetWindow not available", instance, widgetInstance.getWidgetInstance());
                            }
                        }, 10000); // Adjust delay as needed to ensure the widget is ready.
                        return widgetInstance;
                    };
                    resolve(wrappedInit);
                    return;
                }
                if (count > 200) { // Timeout after 20 seconds (200 * 100ms)
                    scriptLoadingStatus.isLoading = false;
